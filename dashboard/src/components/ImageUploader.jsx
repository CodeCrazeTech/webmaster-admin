...
function ImageUploader({ onImageDataChange, defaultPreview }) {
  const [preview, setPreview] = useState(null);
  const [dragging, setDragging] = useState(false);

  useEffect(() => {
    if (defaultPreview) {
      setPreview(API_DOMAIN + defaultPreview);
    } else {
      setPreview(null);
    }
  }, [defaultPreview]);
    ...
    <div className="flex items-center justify-center w-full">
      ...
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          {preview ? (
            <img src={preview} alt="Preview" className="w-60 h-60 object-contain" />
          ) : (
            <>
              <svg
